...
func buildCloudContact(phone, name string) map[string]any {
	...
	if phone != "" {
		contact["phones"] = []map[string]any{
			{"phone": phone, "wa_id": strings.TrimPrefix(phone, "+")},
		}
	}
	return contact
}
...
func (s *Service) postToChatwootCloud(ctx context.Context, cfg *Config, payload any) error {
	jsonData, err := json.Marshal(payload)
	if err != nil {
		return fmt.Errorf("failed to marshal cloud webhook payload: %w", err)
	}

	url := strings.TrimRight(cfg.URL, "/") + "/webhooks/whatsapp"
...
func (s *Service) handleMessageCloud(ctx context.Context, cfg *Config, payload []byte) error {
	...
	if mediaInfo != nil {
		...
		link, err := s.downloadAndUploadCloudMedia(ctx, cfg, data, mediaInfo, msgID)
		if err != nil {
	...
	if err := s.postToChatwootCloud(ctx, cfg, envelope); err != nil {
...
func (s *Service) downloadAndUploadCloudMedia(ctx context.Context, cfg *Config, data *waMessagePayload, info *mediaInfo, msgID string) (string, error) {
	...
	if mimeType == "" {
		mimeType, _ = DetectMIME("", mediaData)
	}
