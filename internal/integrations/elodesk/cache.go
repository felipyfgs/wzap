...
type Cache interface {
	GetConv(ctx context.Context, sessionID, chatJID string) (convID, contactID int64, ok bool)
	SetConv(ctx context.Context, sessionID, chatJID string, convID, contactID int64)
...
func (r *RedisCache) GetConv(ctx context.Context, sessionID, chatJID string) (int64, int64, bool) {
	key := fmt.Sprintf("el:conv:%s:%s", sessionID, chatJID)
...
func (r *RedisCache) SetConv(ctx context.Context, sessionID, chatJID string, convID, contactID int64) {
	key := fmt.Sprintf("el:conv:%s:%s", sessionID, chatJID)
...
func (r *RedisCache) GetIdempotent(ctx context.Context, sessionID, sourceID string) bool {
	key := fmt.Sprintf("el:idempotent:%s:%s", sessionID, sourceID)
...
func (m *MemoryCache) GetConv(_ context.Context, sessionID, chatJID string) (int64, int64, bool) {
...
func (m *MemoryCache) SetConv(_ context.Context, sessionID, chatJID string, convID, contactID int64) {
	m.mu.Lock()
	defer m.mu.Unlock()
...
func NewCache(ctx context.Context, redisURL string) Cache {
	...
	if err != nil {
		logger.Warn().Str("component", "elodesk").Err(err).Str("redisURL", redisURL).Msg("invalid Redis URL, using in-memory cache")
		return newMemoryCache(ctx)
	}
	...
	if err := client.Ping(pingCtx).Err(); err != nil {
		logger.Warn().Str("component", "elodesk").Err(err).Msg("Redis unavailable, using in-memory cache")
		_ = client.Close()
		return newMemoryCache(ctx)
	}
	logger.Info().Str("component", "elodesk").Str("addr", opt.Addr).Msg("Redis cache connected")
