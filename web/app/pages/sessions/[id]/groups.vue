  ...
  <UDashboardPanel id="session-groups">
    <template #header>
      <UDashboardNavbar title="Groups">
        ...
        <template #right>
          <UButton icon="i-lucide-refresh-cw" color="neutral" variant="ghost" size="sm" :loading="loading" @click="fetchGroups" />
        </template>
      </UDashboardNavbar>
      ...
      <UDashboardToolbar>
        <template #left>
          ...
          <USelect v-model="typeFilter" :items="typeFilterOptions" value-key="value" size="sm" class="w-36" />
        </template>
        <template #right>
          ...
          <UButton icon="i-lucide-link" label="Join" size="sm" color="neutral" variant="outline" @click="joinOpen = true" />
          <UButton icon="i-lucide-network" label="New Community" size="sm" color="neutral" variant="outline" @click="communityOpen = true" />
          <UButton icon="i-lucide-plus" label="New Group" size="sm" color="primary" @click="createOpen = true" />
        </template>
      </UDashboardToolbar>
    </template>
    ...
    <template #body>
      ...
      <template v-else>
        <p class="text-xs text-muted mb-3">{{ filtered.length }} group(s)</p>
          ...
          <template #default="{ item: group }">
            ...
            >
              ...
              <div class="flex-1 min-w-0">
                <p class="text-sm font-medium text-highlighted truncate">{{ group.name || 'Unnamed' }}</p>
          ...
          <template #default="{ item: group }">
            ...
            >
              <div class="flex items-center gap-2 mb-2">
                ...
                <p class="font-medium text-sm text-highlighted truncate flex-1">{{ group.name || 'Unnamed' }}</p>
              </div>
        ...
        <template #body>
          <div class="space-y-4">
            ...
            <UFormField label="Participants" description="Phone numbers separated by commas or new lines.">
              <UTextarea v-model="createParticipants" placeholder="5511999999999&#10;5511888888888" :rows="3" class="w-full" />
            </UFormField>
          </div>
        </template>
        <template #footer>
          <div class="flex justify-end gap-2">
            <UButton label="Cancel" color="neutral" variant="subtle" @click="createOpen = false" />
            <UButton label="Create" color="primary" :loading="creating" @click="createGroup" />
          </div>
        </template>
      </UModal>
      ...
      <UModal v-model:open="joinOpen" title="Join Group" description="Enter an invite code to preview and join a group.">
          ...
          <div class="space-y-4">
            <UFormField label="Invite code" required>
              <div class="flex gap-2">
                ...
                <UButton label="Preview" size="sm" color="neutral" variant="outline" :loading="previewing" @click="previewInvite" />
              </div>
            </UFormField>

            <div v-if="joinPreview" class="rounded-lg border border-default p-3 space-y-1.5">
              <p class="font-medium text-highlighted">{{ joinPreview.name }}</p>
              <p v-if="joinPreview.topic" class="text-sm text-muted">{{ joinPreview.topic }}</p>
        ...
        <template #footer>
          <div class="flex justify-end gap-2">
            <UButton label="Cancel" color="neutral" variant="subtle" @click="joinOpen = false; joinPreview = null" />
            <UButton label="Join" color="primary" :loading="joining" @click="joinGroup" />
          </div>
        </template>
      </UModal>
        ...
        <template #body>
          <div class="space-y-4">
            ...
            <UFormField label="Description">
              <UTextarea v-model="communityDesc" placeholder="What is this community about?" :rows="3" class="w-full" />
            </UFormField>
          </div>
        </template>
        <template #footer>
          <div class="flex justify-end gap-2">
            <UButton label="Cancel" color="neutral" variant="subtle" @click="communityOpen = false" />
            <UButton label="Create" color="primary" :loading="creatingCommunity" @click="createCommunity" />
          </div>
        </template>
      </UModal>
    </template>
